---
title: "采样"
description: "让你的服务器从LLM请求补全"
---

采样是MCP的一个强大功能，它允许服务器通过客户端请求LLM补全，实现复杂的代理行为，同时保持安全性和隐私性。

<Info>
  Claude Desktop客户端目前尚不支持MCP的这项功能。
</Info>
...
`messages`数组包含要发送给LLM的对话历史。每条消息都有：
...
可选的`systemPrompt`字段允许服务器请求特定的系统提示词。客户端可以修改或忽略这个提示词。
